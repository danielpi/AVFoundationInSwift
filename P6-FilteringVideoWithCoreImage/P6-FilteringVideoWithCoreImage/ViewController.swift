...
enum OSTypedEnum: OSType {
    ...
    var fourccRepresentation: String {
        if self.rawValue <= 0x28 {
            ...
            return charsToString(UInt32ToChars(self.rawValue))
        }
    }
    
}

func printAllPixelFormats() {
    // Get an array of all pixel format numbers (OSType is a UInt32). Convert them to the fourcc representation
    // if appropriate and print that out. This should include all Pixel Formats that the operating system knows
    // about (likely to change when macos is updated).
    ...
    for format in pixelFormatDescriptions {
        if format <= 0x28 {
            ...
            print("Core Video Pixel Format Type (FourCC): \(charsToString(UInt32ToChars(format)))")
        }
    }
}

func UInt32ToChars(_ input: UInt32) -> [UInt8] {
    var result: [UInt8] = []
    for i in 0..<4 {
        result.append(UInt8(input >> UInt32(8 * i) & UInt32(0xFF)))
    }
    return result.reversed()
}

func charsToString(_ charArray: [UInt8]) -> String {
    return charArray.reduce("") { $0 + String(Character(UnicodeScalar($1))) }
}
...
extension OSType {
    ...
    var fourccRepresentation: String {
        if self <= 0x28 {
            ...
            return charsToString(UInt32ToChars(self))
        }
    }
